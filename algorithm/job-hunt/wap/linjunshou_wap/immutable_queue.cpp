...
public:
  ...
  immutable_queue<T> append(immutable_queue<T> & rhs) {
...
T immutable_queue<T>::getMax()
{
  ...
  T res = front_.getMax();
  if(!rear_.empty()) res = max(res, rear_.getMax());
  return res;
}
...
int main(void)
{
  ...
  immutable_queue<int> p2 = p1.enqueue(0);
  p2 = p2.enqueue(4);
  p2 = p2.enqueue(5);
  p2 = p2.enqueue(6);
  assert(p2.size() == 7);
  ...
  immutable_queue<int> temp = p2.reverse();
  while(!temp.empty())
  {
    cout << temp.peek() << " ";
    temp = temp.dequeue();
